...
public class ArscEditor extends AssetEditor {
    ...
    private static def LIBRARY_ENTRY_SIZE = 260
    // packageId(4), packageName(256)
    private static def LIBRARY_CHUNK_SIZE = 272
    // ResTable_lib_header & ResTable_lib_entry
    private static def TABLE_SIZE_POS = 4

    private int mTableConfigSize = 52
    ...
    def slice(int pp, Map idMaps, Map libRefTable, List retainedTypes) {
        ...
        if (retainedTypes[0].id != 1) {
            // attr type id is always as `1'
        ...
        retainedTypes.each {
            ...
            ts.configs.each {
                ...
                es.each { e ->
                    ...
                    def ename = new String(t.keyStringPool.strings[entry.key]).replaceAll('\\.',
                        '_')
                    if (e.name != ename) {
                        throw new Exception(
                            "Required entry '${e.name}' but got '$ename', This " + "is seems to unsupport the buildToolsRevision: ${version}.")
                    }
        ...
        retainedEntries.each { e -> e.key = keyMaps[e.key]
        }
        ...
        libPackageIds.each { pid ->
            ...
            if (pname == null) {
                ...
                libRefTable.each {
                    id, name -> err += "  [${String.format('0x%02x', id)}] -> $name\n"
                }
        ...
        size +=
            t.typeStringPool.header.size + t.keyStringPool.header.size + t.package.header.headerSize
    ...
    def reset(int pp, Map idMaps) {
        ...
        def pkgSizePos = tellp() - 4
        // for adding size after insert dynamicRefTable
    ...
    private def dumpTable(t) {
        ...
        if (lib != null) {
            ...
            lib.entries.each { e ->
                println "    0x${Integer.toHexString(e.packageId)} -> " + "${getUtf8String(e.packageName)}"
            }
            println ''
        }
